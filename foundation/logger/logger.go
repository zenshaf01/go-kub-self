...
func (log *Logger) Info(ctx context.Context, msg string, args ...any) {
	...
	// the caller being passed a 3 is used to get the location of where this log has originally come from.
...
func (log *Logger) write(ctx context.Context, level Level, caller int, msg string, args ...any) {
	...
	// construct an uintptr to represent a slice of program counters
	var pcs [1]uintptr

	// Calling Callers fills the pcs slice with the program counters for the function invocations.
	// This essentially tells us where the log is coming from
	// the caller value is the place where the Callers should start filling the slice
	// 3 would mean go back 3 invocation starting from 0 where 0 is inside Callers() and 1 being this caller function write().
